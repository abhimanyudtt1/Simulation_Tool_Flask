...
def deployOnProd():
    ...
    import subprocess
    (stdout,stderr) = subprocess.Popen('sh /usr/local/projects/branch_merge.sh %s %s' % (component,branch),stderr=subprocess.PIPE,stdout=subprocess.PIPE,shell=True).communicate()
    stdout = stdout.split('\n')
    stderr = stderr.split('\n')
...
def tripSimulation():
    ...
    print request.form
    global THREAD_ID
    print "ASDFFADSFASD : %s " % THREAD_ID
    THREAD_ID += 1
    tripid = simulator.DriverTripAllocation(importer.driverIds, importer.routeIds, 0,importer.optionsRadios)
...
def runDriverSimulation():
    ...
    import simulator
    tripid = simulator.DriverTripAllocation(driverIds, RouteIds, int(time),env)
    return render_template('result.html',result =[driverIds, RouteIds,tripid] )
